...
- Session state management: User sessions, including shopping cart data, are managed by Azure Managed Redis, ensuring persistence and consistency across scale-out events.
...
- API interaction and token management: The front-end web app uses the MSAL library to obtain tokens for authenticated API calls, caching them in Azure Managed Redis to optimize performance and manageability.
