...
- The architecture uses a hub-and-spoke network, Azure Front Door, App Service, Azure SQL, Azure Cache for Redis, Azure Key Vault, App Configuration, and Application Insights.
...
- **Test/validate**: Use scripts in `testscripts/` (e.g., `setup.ps1`, `validate-deployment.ps1`, `cleanup.ps1`).
...
- **Session State**: Uses Azure Cache for Redis for distributed session and token caching.
